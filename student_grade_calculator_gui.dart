...
class Student {
  ...
  bool addGrade(double grade) {
    if (grade >= 0 && grade <= 100) { grades.add(grade); return true; }
    return false;
  }
}
...
String toCsv(List<Student> students) {
  ...
  for (final s in students) {
    buf.write('${s.id},${s.name},"${s.grades.join(';')}",'
...
class _GradeHomePageState extends State<GradeHomePage> {
  ...
  List<Student> get _displayed {
    if (_filter == 'Passing') return filterStudents(_students, (s) => s.isPassing());
    if (_filter == 'Failing') return filterStudents(_students, (s) => !s.isPassing());
    return _students;
  }
  ...
  void _addStudent() {
    ...
    setState(() {
      ...
      _idCtrl.clear(); _nameCtrl.clear(); _gradesCtrl.clear();
    });
  }
  ...
  void _showExport(String type) {
    String content;
    switch (type) {
      case 'HTML':  content = toHtml(_students); break;
      case 'XML':   content = toXml(_students);  break;
      default:      content = toCsv(_students);  break;
    }
    showDialog(
      context: context,
      builder: (_) => AlertDialog(
        backgroundColor: AppColors.midBlue,
        title: Text('$type Export Preview', style: const TextStyle(color: AppColors.orange)),
        content: SizedBox(
          width: 500, height: 350,
          child: SingleChildScrollView(
            child: SelectableText(content,
                style: const TextStyle(color: AppColors.white, fontFamily: 'monospace', fontSize: 12)),
          ),
        ),
        actions: [
          TextButton(
            ...
            children: [
              ...
              Row(children: [
                ...
                Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  const Text('Student Grade Calculator',
                      style: TextStyle(color: AppColors.orange, fontSize: 26, fontWeight: FontWeight.bold)),
                  Text('Project Milestone 2 — Flutter GUI',
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(children: [
                      ...
                      const SizedBox(width: 10),
                      _field(_gradesCtrl, 'Grades (comma separated)', flex: 3),
                      const SizedBox(width: 10),
                      ElevatedButton(
                    ...
                    if (_statusMsg.isNotEmpty) ...[
                      ...
                      Text(_statusMsg,
                          style: TextStyle(
                              color: _statusMsg.startsWith('✓') ? AppColors.greenPass : Colors.yellow,
                              fontSize: 13)),
                    ],
                  ],
                ),
              ...
              // Filter + Export
              Row(children: [
                const Text('Filter:', style: TextStyle(color: AppColors.gray)),
                const SizedBox(width: 8),
                ...['All', 'Passing', 'Failing'].map((mode) => Padding(
                  padding: const EdgeInsets.only(right: 6),
                  child: ElevatedButton(
                    onPressed: () => setState(() => _filter = mode),
                    style: ElevatedButton.styleFrom(
                      backgroundColor: _filter == mode ? AppColors.orange : AppColors.midBlue,
                      shape: const StadiumBorder(),
                    ),
                    child: Text(mode, style: const TextStyle(color: Colors.white, fontSize: 13)),
                  ),
                )),
                const Spacer(),
                const Text('Export:', style: TextStyle(color: AppColors.gray)),
                const SizedBox(width: 8),
                ...['HTML', 'XML', 'Excel'].map((t) => Padding(
                  padding: const EdgeInsets.only(left: 6),
                  child: OutlinedButton(
                    onPressed: () => _showExport(t),
                    style: OutlinedButton.styleFrom(
                        side: const BorderSide(color: AppColors.orange)),
                    child: Text(t, style: const TextStyle(color: AppColors.orangeLight)),
                  ),
                )),
              ]),
              const SizedBox(height: 12),
              ...
              Container(
                ...
                child: Row(children: [
                  for (final h in ['ID', 'Name', 'Grades', 'Avg', 'Grade', 'Status'])
              ...
              Expanded(
                child: ListView.builder(
                  ...
                  itemBuilder: (_, i) {
                    ...
                    final bg  = i.isEven ? AppColors.midBlue : AppColors.darkBlue;
                    final sc  = s.isPassing() ? AppColors.greenPass : AppColors.redFail;
                    return Container(
                      color: bg,
                      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10),
                      child: Row(children: [
                        Expanded(child: Text(s.id, style: const TextStyle(color: AppColors.gray, fontSize: 13))),
                        Expanded(child: Text(s.name, style: const TextStyle(color: Colors.white))),
                        Expanded(child: Text(s.grades.map((g) => g.toInt()).join(', '),
                            style: const TextStyle(color: AppColors.gray, fontSize: 12))),
                        Expanded(child: Text(s.calculateAverage().toStringAsFixed(2),
                            style: const TextStyle(color: AppColors.orangeLight, fontWeight: FontWeight.bold))),
                        Expanded(child: Text(s.getLetterGrade(),
                            style: const TextStyle(color: AppColors.orange, fontWeight: FontWeight.bold, fontSize: 18))),
                        Expanded(child: Text(s.isPassing() ? 'PASS' : 'FAIL',
              ...
              Container(
                decoration: BoxDecoration(
                  color: AppColors.midBlue, borderRadius: BorderRadius.circular(8)),
                padding: const EdgeInsets.all(14),
                child: Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
                  _stat('${allStudents.length}', 'Students'),
                  _stat(classAvg.toStringAsFixed(1), 'Class Avg'),
                  _stat('${filterStudents(allStudents, (s) => s.isPassing()).length}', 'Passing',
